# Exact E-loss + TD loss using the sampling form (Dirichlet / spectral decomposition)
# for Evaluation of a pretrained fixed policy
...
def make_train(base_config):    
    ...
    def train(rng, hparams=None):
        ...
        def td_loss(params):
            ...
            e = V - v
            # Sampling form:
            # E = (1 - γ) * ||e||_D^2 + (γ / 2) * sum_{ij} mu_i * p_{ij} * (e_i - e_j)^2
            magnitude_term = (1.0 - γ) * (e ** 2)
            laplacian_term = 0.5 * γ * jnp.sum(P_π * ((e[:, None] - e[None, :]) ** 2), axis=1)

            TD_targets = R_π + γ * P_π @ v
            td_errors = v - jax.lax.stop_gradient(TD_targets)
            
            total_loss = 0.5 * jnp.sum(mu * (magnitude_term + laplacian_term + td_errors**2))
            return total_loss
