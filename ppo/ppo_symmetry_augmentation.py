...
SAVE_DIR = "ppo_symmetry_augmentation"
...
def make_train(config):
    ...
    def train(rng):
        ...
        def _update_step(runner_state, unused):
            ...
            # COLLECT TRAJECTORIES, randomly flipping observations
            def _env_step(env_scan_state, unused):
                train_state, env_state, last_obs, rng = env_scan_state

                # 1. Randomly flip the CURRENT observation
                rng, _rng_flip = jax.random.split(rng)
                flip_mask = jax.random.bernoulli(_rng_flip, p=0.5, shape=(config["NUM_ENVS"], *([1] * len(obs_shape))))
                # Convert boolean mask to 1 or -1
                multiplier = 1.0 - 2.0 * flip_mask.astype(jnp.float32)
                augmented_obs = last_obs * multiplier

                rng, _rng = jax.random.split(rng)
                # Use augmented_obs for the policy/value forward pass
                pi, value = network.apply(train_state.params, augmented_obs)
                ...
                # 2. Augment the NEXT observation for consistent bootstrapping
                true_next_obs = info['real_next_obs']
                # Note: Usually, you apply the SAME flip to the next_obs 
                # so the transition (s, a, s') remains consistent.
                augmented_next_obs = true_next_obs * multiplier
                
                next_val = network.apply(train_state.params, augmented_next_obs, method=network.value)

                transition = Transition(
                    done, action, value, next_val, reward, log_prob, augmented_obs, info
                )
