# Based on random_policy.live_iv_mc_phi.py
# allows gradients through w for both monte carlo prediction and the second stage prediction.
# allows gradients through phi for only monte carlo prediction - not the two stage instrument tasks.
...
def iv_loss_fn(params, network, traj_batch, advantages, targets, config):
    ...
    # =========================================================
    z = network.apply(
        params, traj_batch.obs, method=network.z
    )  # z is seperate from phi. it is differentiable for next feature prediction.
    ...
    x_target = jax.lax.stop_gradient(phi - config["GAMMA"] * phi_next * not_done)
    ...
    loss_dict = {"forward_loss": forward_loss, "done_loss": done_loss, "reward_loss": reward_loss, "v_loss": v_loss}
...
def make_train(config):
    ...
    def train(rng):
        ...
        network = IVActorCritic(action_dim=action_dim, is_continuous=False, z_dim=k, phi_dim=k, norm_type=norm_type)
        network_params = network.init(init_rng, jnp.zeros(obs_shape), jnp.zeros(action_dim))
        train_state = networks.initialize_flax_train_state(
            config,
            network,
            network_params,
        )
        ...
        def _update_step(runner_state, unused):
            ...
            def _env_step(env_scan_state, unused):
                ...
                transition = Transition(done, action, value, next_val, reward, log_prob, last_obs, true_next_obs, info)
