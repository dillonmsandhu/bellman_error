...
# def loss_fn(params, network, traj_batch, gae, targets, traces, config):
#     v_loss = helpers.v_loss_fn(params, network, traj_batch, gae, targets, config)
#     trace_pred = network.appy(params, traj_batch.obs)
#     trace_mse = (traces -) 
...
def make_train(config):
    ...
    def train(rng):
        k = config.get('k', 32)
        network, network_params = networks.initialize_network(rng, obs_shape, env, env_params, k, n_heads=1)
        ...
        def _update_step(runner_state, unused):
            ...
            traces = helpers.calculate_traces(traj_batch, config["GAMMA"], config["GAE_LAMBDA"])
