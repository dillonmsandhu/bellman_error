...
SAVE_DIR = "random_policy_fixed_z_mc_phi_dummy"
...
class IVActorCritic(nn.Module):
    ...
    def setup(self):
        ...
        self.v_shape_head = nn.Dense(1, kernel_init=orthogonal(0.01))
    ...
    def shape_value(self, obs):
        phi_curr = self.phi(obs)
        return self.v_shape_head(phi_curr).squeeze(-1)
    ...
    def __call__(self, obs, action=None):
        ...
        phi_curr = self.phi(obs)
        value = self.w(phi_curr).squeeze(-1)
        value_shaping = self.v_shape_head(phi_curr).squeeze(-1)
...
def v_shape_loss_fn(params, network, traj_batch, targets, config):
    # This ONLY touches phi_net and v_shape_head!
    value_pred = network.apply(params, traj_batch.obs, method=network.shape_value)
...
def iv_loss_fn(params, network, traj_batch, advantages, targets, config):
    ...
    # Doesn't flow gradient through critic weights, only φ.
    # v_loss = helpers.no_w_v_loss_fn(params, network, traj_batch, advantages, targets, config)
    v_loss = v_shape_loss_fn(params, network, traj_batch, targets, config)
    # =========================================================
