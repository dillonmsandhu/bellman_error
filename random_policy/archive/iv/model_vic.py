...
def iv_loss_fn(params, network, traj_batch, advantages, targets, config):
    ...
    a = config.get("VICREG_PHI_WEIGHT", 0.01)
    ...
    total_loss = forward_loss + done_loss + reward_loss + loss_actor - (config["ENT_COEF"] * entropy) + vic_loss
