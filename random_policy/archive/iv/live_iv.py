# Uses monte carlo learning and instrumental variables to update w.
...
def iv_loss_fn(params, network, traj_batch, advantages, targets, config):
    ...
    # Gradient flows through all params.
    ...
    ent_coeff = config["ENT_COEF"]
    mc_coeff = config.get('MC_COEF', 0.1)
    total_loss = forward_loss + done_loss + reward_loss + loss_actor - (ent_coeff * entropy) +  mc_coeff * v_loss
