...
SAVE_DIR = "random_iv_model_vic_mc"
...
def iv_loss_fn(params, network, traj_batch, advantages, targets, config):
    ...
    x_hat, done_logit = network.apply(params, jax.lax.stop_gradient(z), method=network.g_A)
    ...
    a = config.get("VICREG_PHI_WEIGHT", 0.1)
    ...
    total_loss = forward_loss + done_loss + reward_loss + loss_actor - (config["ENT_COEF"] * entropy) + vic_loss + 0.1 * v_loss
...
def make_train(config):
    ...
    def train(rng):
        ...
        network = IVActorCritic(
            ...
            is_continuous=False, z_dim=k, phi_dim=k, norm_type = norm_type
        )
